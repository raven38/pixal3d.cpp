...
Each `<name>/` directory tracks only the 4096² preview grid; the source images and the meshes
are attached to the [`showcase-assets-sv-1` release](https://github.com/raven38/pixal3d.cpp/releases/tag/showcase-assets-sv-1) instead of the git history, so a clone
stays small (they were removed from the history on 2026-09-24 together with the rest of the
repository's binary ballast):

- `<name>_quad4k.webp` — the four-view preview grid (tracked here)
- `<name>.png` (release asset) — the source image fed to the pipeline (RGB, no alpha)
- `<name>.glb` (release asset) — the textured PBR mesh output, stored **Draco-compressed**
