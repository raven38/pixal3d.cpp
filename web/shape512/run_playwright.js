// Drives web/shape512/index.html in a real Chrome (WebGPU + JSPI) and saves the report and the
// latents the module returned. Usage (from web/, with `python3 -m http.server 8199` serving it):
//   node shape512/run_playwright.js <dinov3.gguf> <pixal3d_naf.gguf> <pixal3d_shape_flow_512_mv.gguf>
//                                   <cond_slat_dir> <slat_sample_dir> <out_dir> [views] [own_cond]
...
// passed (not the 1.1 GB s512_naf_hr_v0.npy nor the s1024_* files). Needs `playwright` resolvable
...
(async () => {
  ...
  const condFiles = listNpy(condDir, f => !/naf_hr|s1024_/.test(f));
  const sampleFiles = listNpy(sampleDir, f => !/dec_|cuda_|cpp_/.test(f));
  ...
  await page.goto('http://localhost:8199/shape512/index.html', { waitUntil: 'load' });
