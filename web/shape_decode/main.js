// Page side of the browser shape-decode harness: collect File objects, spawn the worker, print
// the report, offer the returned mesh files for download. No pipeline logic here.
(() => {
  ...
  document.getElementById('run').addEventListener('click', () => {
    ...
    if (!model || fixture.length === 0) { log('need shape_dec.gguf and the fixture files'); document.title = 'RESULT: FAIL'; return; }
    log('navigator.gpu present (page): ' + (!!navigator.gpu));
    log('starting worker with ' + model.name + ' and ' + fixture.length + ' fixture files, res=' + res);
    ...
    worker.onmessage = (ev) => {
      ...
      if (m.type === 'done') {
        ...
        for (const name of Object.keys(m.files)) {   // browser_verts.npy etc.; the playwright driver saves these
    ...
    worker.postMessage({ model, fixture, res });
  });
})();
