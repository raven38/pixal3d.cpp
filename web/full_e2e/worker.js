...
self.onmessage = async (ev) => {
  const { models, fixture, seed, texCondFixture } = ev.data;
  ...
  try {
    ...
    const args = need.map(n=>'/models/'+n).concat(['/fixture','/out/full_fixture.glb',seed||1,texCondFixture?1:0]);
    const report = await Module.ccall('pixal3d_full_fixture_run','string',Array(11).fill('string').concat(['number','number']),args,{async:true});
