...
#endif
 Pixal3dInputViews in;string err;if(!pixal3d_load_input_views(views,in,err)){rlog("input error: %s\n",err.c_str());return 2;}rlog("real input: V=%zu mesh_scale=%.4f%s\n",in.views512.size(),in.mesh_scale,
      in.has_mesh_scale?"":"  ** transforms.json に mesh_scale が無い（既定 1.0）**");
 if(!in.has_mesh_scale)
  rlog("WARNING: mesh_scale が指定されていません。ビューのレンダ時のスケールと食い違うと\n"
       "         ProjGrid が被写体を間違ったスケールでサンプリングし、エラーを出さずに\n"
       "         壊れた形状になります（実測: シルエット IoU 0.909 -> 0.107）。\n"
       "         tools/silhouette_iou.py で整合する値を推定できます。\n");
