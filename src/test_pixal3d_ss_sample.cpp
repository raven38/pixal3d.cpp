...
//   trellis-test-pixal3d-ss-sample <pixal3d_ss_flow_mv.gguf> <ss_dec.gguf> <fixture_dir> [gpu] [cond_dir]
...
int main(int argc, char** argv) {
    if (argc < 4) {
        fprintf(stderr, "usage: %s <pixal3d_ss_flow_mv.gguf> <ss_dec.gguf> <fixture_dir> [gpu] [cond_dir]\n", argv[0]);
        return 1;
    }
    const string gguf_flow = argv[1], gguf_dec = argv[2], fdir = argv[3];
    const int gpu = argc > 4 ? atoi(argv[4]) : 0;
    string cond_dir;
    if (argc > 5) cond_dir = argv[5];
    else if (const char* e = getenv("PIXAL3D_COND_DIR")) cond_dir = e;
    else cond_dir = fdir + "/../cond_ss";
    printf("fixture_dir=%s gpu=%d (cond fallback dir=%s)\n", fdir.c_str(), gpu, cond_dir.c_str());
    ...
    // ---- per-step latent parity vs f32 and bf16 refs ----
    ...
    trellis::Model md = trellis::Model::load(gguf_dec, gpu);
