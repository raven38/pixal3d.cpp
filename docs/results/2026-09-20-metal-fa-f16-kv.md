...
- **数値: Metal では現行既定の bf16 K/V が最も精度が低い。** op 単体（float64 参照、§3）で bf16 7.5e-3 / f16 3.7e-4 /
  exact 4.5e-4。実 conditioning の tex flow（§7、seed 5、paired、再実行 bit-exact）で bf16 は exact-softmax 経路から
  rms 3.0–3.8% 逸脱、f16 は 0.02%。E2E の SS latent（§6.3）でも bf16 4.4e-2 vs f16 6.4e-3。機構は Metal カーネルの
  型指定（bf16: q が bfloat + 出力累積 half、§1）で、CUDA の「bf16 が安全」とは逆。
- **範囲: F16 の overflow リスクは無い。** Q/K は RMSNorm×gamma の静的上界（≤ 206、§2）で入力非依存、V は cyclops
  1 入力で headroom ≥ 12646×（§4）。NaN/Inf は全経路 0。
...
- **判断はユーザーへ**: 精度目的で Metal の既定を f16 にするか（速度中立〜Texture 4.5% 減、精度 op 単体 20 倍・
  flow 100 倍以上、V の範囲は 1 入力のみ確認）。変えるなら別 issue/PR で 2 入力目の範囲計測と holdout E2E を付ける。
  WebGPU 経路（BF16 FA が無く f16 必須）にとっては、この結果は f16 K/V の精度リスクが低いことの裏付けになる。
...
nonfinite は全構成 0。CPU の exact 経路は真の F32 参照（1.4e-7）だが、**Metal の exact 経路は F32 参照ではない**:
ggml-metal の `kernel_mul_mm_f32_f32`（`ggml-metal.metal` L10205）は両オペランドを threadgroup メモリに `half` で
置いて `simdgroup_half8x8` で積和するので、`--no-fa` も half オペランド + float 累積の経路になり、FA f16 K/V と同じ
4e-4 級に落ちる（テストの `FA_ORACLE_EXACT_REF` はこれを検出して NOT-F32 を出す。`FA_ORACLE_RESULT` 自体は OK）。

読み取り:
- Metal では **bf16 K/V（現行既定）が float64 から最も遠い**（flat 7.5e-3、peaked 8.6e-3）。f16 K/V はその **1/20〜1/8**
  で、Metal の「exact」経路や f32 K/V と同じ桁。§1 のカーネル仕様（bf16: q が bfloat + O 累積 half）どおりの結果。
- peaked では bf16 の max|d| が 0.35（ref の rms は ~4）で、softmax が尖った query で局所的に大きく外す。
...
| Texture | 17612 | 5 / 5 | 13.96 s | 14.63 s | 27.36 s | **0.955 [0.922, 0.989]**（f16 が 4.5% 遅い、有意） | 0.511 [0.486, 0.537] |
...
**H1（f16 が速い）は M4 Max では成立しない**: Shape-1024 は差なし、Texture は f16 が有意に遅い。f32 K/V は 2 倍遅い
（f32 カーネルは K/V を float の simdgroup 行列で読むぶん帯域が倍になる）。**ゲート 4（速度）は不通過**。
...
`trellis_cli.cpp` の flow noise は 1 本の `mt19937(seed)` から voxel の座標順に `N×32` 個ずつ引く（L393/L789 の
`noise(n)`）。そのため **SS の active voxel 集合が 1 voxel でも違うと、その行以降の voxel は別の noise を受け取り**、
下流の LR / HR / tex flow は「同じ seed だが別サンプル」になる。bf16 と f16 の E2E で確認:

- `ss_coords`: 4435 / 4438 voxel が共通（Jaccard 0.9986）。差は 3 voxel。
- `shape_noise.npy`（HR 段の noise、行番号で保存）は **行単位では 17614 行すべて一致**するが、座標で対応付けると
  共通 17240 voxel のうち **同じ noise を受けたのは 767 voxel だけ**（ref 順の 176 番目から 1 行ずれる）。
- 結果、`lr_slat` は共通 voxel 上で rel_rms 0.70、`hr_slat` / `tex_slat` は **1.41 ≈ √2**（独立な 2 サンプルの距離）。

したがって E2E レベルで K/V 型の影響を要素比較できるのは **SS latent（dense 16³、noise が同一）だけ**で、
HR / tex の要素比較は Q2（§7、座標と noise を fixture で固定）が正しい設計。設計 3b の Jaccard / GLB IoU は
この「別サンプル」効果を含んだ粗いゲートとして読む。（noise を座標ハッシュで引けばこの結合は切れるが、
参照実装との seed 互換に関わるので本 issue では触らない。）

### 6.3 3 本の比較（ref = no-fa、exact softmax・half オペランド）
...
- **設計 3b（HR voxel 集合の Jaccard ≥ 0.95）は両腕とも下限を割り、判定不能**: 6.2 の noise 行番号結合で、SS の
  voxel 集合が 2〜4 個違うだけで HR 段が「同じ conditioning の別サンプル」になる（Jaccard 0.92 は独立サンプル間の
  値）。設計時の前提「coords はほぼ一致する」が崩れたので、3b は K/V 型の検定として成立しない。GLB の IoU / V/F /
...
`err_arm = rms(x_final_arm − x_final_nofa)` を取る。参照の `--no-fa` は **softmax 厳密・オペランド half の経路**
（§3: Metal では F32 参照ではない）なので、f16–nofa の差は参照自身の誤差水準と分解できない。判定は paired の
...
- **paired n=5: mean d = −5.06（比 0.006）、se 0.081、片側 95% CI 上限 −4.89（比 0.008）** → f16 K/V は bf16 K/V より
  exact-softmax 参照に **100 倍以上**近い（5/5 seed で同方向、ばらつきは小さい）。
- スケール感: seed 間の rms 距離（nofa 同士）は 0.45–0.50 なので、bf16 の逸脱 0.019 は「別 seed の出力」の 4% 相当。
- step ごとの平均相対誤差（5 seed 平均）: bf16 は step 1 の 1.2e-4 から step 12 の 3.4e-2 まで **1 step あたり約 ×1.5**
  で増幅する（flow の後半ほど入力感度が高い）。f16 は 5e-6 → 2.2e-4 で、増幅率は同じだが出発点が 20 倍小さい。
  op 単体で見た ×20（§3）が、12 step の flow を通ると ×160 に開く。
...
- **f16 と no-fa の一致（2e-4）の読み方**: 両経路は同じ値を同じ half に丸める（FA f16 の K/V cast と `mul_mm` の
  half ステージング）ので、オペランド丸め誤差を共有し、違いは累積順序だけになる。したがって「f16 が真値に 0.02% まで
  近い」とは言えない。真値からの距離は op 単体（§3）の 3.7e-4（f16）/ 4.5e-4（exact）を flow の増幅率で外挿する
  しかなく、bf16 の増幅（op 7.5e-3 → flow 3.4e-2、×4.5）を当てはめると **f16 / no-fa は真値から ~0.2%、bf16 は
  ~3–4%** という見積もりになる。主張できるのは「bf16 は exact-softmax 経路から 3–4% 離れ、f16 はその経路と
  0.02% で一致し、float64 に対する op 単体の誤差も f16 が bf16 の 1/20」まで。
...
| 3c | Q2 paired n≥5 | **「f16 が bf16 より参照に近い」（上限 < 0）** | §7: mean d = −5.06、片側 95% CI 上限 −4.89（比 0.008）、再実行対は bit-exact |
| 4 | 速度: `t_bf16 / t_f16` の 95% CI > 1 | **不通過** | §5: Shape-1024 0.974 [0.932, 1.018]（差なし）、Texture 0.955 [0.922, 0.989]（f16 が遅い）、n=5 block |
...
| 5 | holdout E2E（auto = f16 で別入力） | **未実施** | 4 が不通過なので既定を変えない → holdout は不要 |
...
(b) **数値**: Metal では現行既定の bf16 K/V が float64 から最も遠い（op 単体 ×8〜20、§3）。実 conditioning の tex flow を
12 step 通すと bf16 は exact-softmax 参照から rms 3.0–3.8% 逸脱し、f16 は 0.02%（§7、5/5 seed、bit-exact 再現）。
逸脱の機構は §1 のカーネル仕様（bf16: q を bfloat で格納 + 出力累積 half）で、CUDA の「bf16 = 安全」とは逆。
**精度目的で Metal の既定を f16 に変えるかはユーザー判断**（速度中立・Texture で 4.5% 減速、精度は 100 倍改善）。
変えるなら別 issue/PR で、holdout E2E（ゲート 5）を付けて行う。
...
**WebGPU への含意**: WebGPU backend は BF16 FA を持たないので browser 経路は必然的に f16 K/V になる
（`docs/PIXAL3D_WEBGPU_OP_GAP.md` B1）。本計測の範囲（≥ 360×）と Q2（f16 ≈ exact）は、その経路の精度リスクが
「overflow」ではなく「V の量子化」であり、それも参照ノイズ以下であることを示す。
...
- Q2 は tex flow のみ（shape flow の HR 再サンプルは 1 run 8 分 × 15 で省略）。op 単体（§3）と tex flow の結果が同方向
  なので shape flow でも同じ傾向が期待されるが、未計測。
