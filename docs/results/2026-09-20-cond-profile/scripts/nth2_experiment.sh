...
OUT=$(dirname $0)/nth2; mkdir -p $OUT
SNAP=$OUT/ps_snapshots.log
others() { ps -Ao pid,etime,args -r | grep -E "trellis|post-replay" | grep -v grep | grep -v "pixal3d-cond-profile/build-metal" | grep -v "nth2_experiment" ; }
...
run "nth1024gn T1024 im2col" $NAF --S 1024 --R 64 --naf-t 1024
run "nth1024gn T1024 im2col" $NAF --S 1024 --R 64 --naf-t 1024
run "nth1024gn T512 im2col"  $NAF --S 1024 --R 64 --naf-t 512
python3 $(dirname $0)/nth_patch.py check > $OUT/patch_diff.txt
