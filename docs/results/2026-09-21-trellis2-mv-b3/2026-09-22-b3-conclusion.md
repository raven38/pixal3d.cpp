...
| TASK-B3-REF 背景の前提「参照側は v2/v3 の stochastic run（2img/4img × 512/1024c）が各 1 seed で全て正常」 | **誤りだった**。R1 の較正ステップで、既存 v3 fixture `run_2img_real_stochastic_1024c`（2-view stochastic、seed 42）の bake 済みテクスチャが**全 texel = 0** と判明（`trimesh` で `baseColorTexture` を直接実測、`np.unique(arr)==[0], max=0`）。`tex_slat` 自体は有限・正常範囲（mean 0.29066333、std 3.5458083、nan/inf 0）で、黒化は decode 以降で発生 | **前提が測定で覆った**（事後変更ではなく、brief の事実誤認が R1 の副産物として判明したもの）。**この覆りが §0 の根拠 1 そのものになっている** — 「参照実装でも黒化が起きる」の実体がこの fixture。**限定**: 前提は 4 構成（2img/4img × 512/1024c）を指すが、較正で bake 済みテクスチャを測り直したのは 1024c の 3 件だけ（`tools/ref_trellis2_mv_seed_sweep.py:60-64` の `V3_MANIFEST_PATHS`。**うち前提が指す 4 構成＝stochastic に当たるのは `run_{2,4}img_real_stochastic_1024c` の 2 件**で、`4v_multidiffusion`（= `run_4img_real_multidiffusion_1024c`）は前提の外側）。つまり**前提の 4 構成のうち 2 件（いずれも 512）は一度も測っていない**。「全て正常」は反例 1 件で偽になるので前提の否定は成立するが、**参照側の黒化が 1024c 固有かどうかは未検証** | R1 doc §3「重要な副産物」、`../2026-09-22-trellis2-mv-b3-ref/raw/sweep_results.jsonl`（2v seed42 の `saturation_rate` 0.9999911） |
...
- **512 構成の確認**: 参照側の掃引・較正はいずれも 1024 cascade のみ。v3 の 512 fixture（2img/4img × 512）の bake 済み
  テクスチャは一度も測っていないので、黒化が 1024c 固有かは分からない。既存 fixture を `trimesh` で読むだけなので
  pod 不要・数分で済む、最も安い追加材料。
