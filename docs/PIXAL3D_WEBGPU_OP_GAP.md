...
3955 nodes/graph (re-measured after PR #48/#52; 4135 before), 30 blocks, `d_model=1536`, `n_heads=12`, `head_dim=128`, proj_attn ON (`d_proj=1024`).
...
> **2026-09-20 更新（PR #48 / #52）**: DiT の 3D RoPE は de-interleaved レイアウトへ書き換えられ (`src/dit.cpp::apply_rope`)、`ARANGE` / `SET_ROWS` / ゼロ埋め `SCALE` と `rope_idx` 入力（および `dit_rope_index` API 自体、#52）は **DiT のグラフから消えた**。**以下の行は #48/#52 後の現状**で、カウントは `TRELLIS_DUMP_OPS` による再計測値（`trellis-test-dit-bench <pixal3d_shape_flow_512_mv.gguf> 4096 1 0`、tag `dit_N4096_dcond5_proj1`、既定の FA 経路、M4 Max / Metal、2026-09-20）。ブロック構成は SS flow DiT と同一で、#48 で変わらない op（`MUL_MAT` 245、`NORM` 91、`RMS_NORM` 120、`ADD` 605、`MUL` 510、`PERMUTE` 240）は本表および §8 の実測と一致する。**旧グラフ（`ARANGE` 122 / `SET_ROWS` 120 / host `rope_idx` 入力の形）の記述は §3 末尾の注記と §8 の実測表にそのまま残る**（当時の実測なので書き換えない）。

| ARANGE | dit.cpp:104 `build_pad_mask` | **2** (was 122; the 120 `apply_rope` index builds are gone since #48) | `[Lk_pad]` = [4096] (self) / [256] (cross, Lc=5) at N=4096, up to ~17,664 at HR | f32 | cont | trivial (≤70 KB) | only the two FA pad-mask ramps (self + cross), built once per graph and shared by all 30 blocks. The RoPE no longer builds any index — cos/sin are host inputs | **C** — `GGML_OP_ARANGE` has **no case at all** in the switch (confirmed absent in both upstream §4 and the fork by direct grep), so each of the 2 occurrences forces a CPU fallback + graph split. Only 2 sync points now, and only on the FA path: the `--no-fa` graph never uses the mask, so it contains no `ARANGE` at all (§8's exact-SDPA histogram has none) — §5/§7 item 1 |
| 3D RoPE elementwise (`RESHAPE`/`PERMUTE`/`CONT`/`VIEW`/`MUL`/`SUB`/`ADD`/`CONCAT`) | dit.cpp:78-94 `apply_rope` (60 calls = 30 blocks × q,k) | per call: RESHAPE 6, PERMUTE 1, CONT 1, VIEW 2, MUL 4, SUB 1, ADD 1, CONCAT 1 → 60 CONCAT / 60 SUB / 240 MUL … per graph | CONT and CONCAT dst f32[64,2,49152] (= `[hd,nh,L]` at N=4096) | f32 | cont / view-cont | 25.2 MB (SS) / ~107 MB (HR, est.) | de-interleaved rotation (#48): the pair-major `CONT` + 4 `MUL`/`SUB`/`ADD` on rows of `half`=64, then one `CONCAT` back to `[hd,nh,L]`. cos/sin are host-built inputs (`gcos`/`gsin`), so there is no index tensor and no scatter | **A** — every one of these is dtype-supported on WebGPU (F32; RESHAPE/VIEW/PERMUTE unconditional, CONT lowers to CPY F32→F32, CONCAT/MUL/SUB/ADD all F32). This is what replaced the pre-#48 `ARANGE`+`SET_ROWS` form, i.e. the C2 gap no longer exists for the RoPE |
| SET_ROWS | — | **0** (was 120, all from `apply_rope`) | — | — | — | — | no `SET_ROWS` node is left in the DiT graph since #48; the op itself is still used elsewhere (sparse decoder) and is dtype-supported | **A** (moot here) |
| SUB / UNARY:STEP / CPY | dit.cpp:88 `apply_rope` (SUB), :105 `build_pad_mask` (STEP), :106 (mask cast) + :242 `prep_kv` (K/V cast) + `rms_gamma` callers (γ cast) | 60 / 2 / **242** (was 362; the 120 `apply_rope` F32→I32 index casts are gone) | CPY dst up to bf16`[128,4096,12,1]` | f32/f16/bf16 | cont/view-cont | up to 25.2 MB (SS) / ~100 MB (HR) | measured CPY breakdown: 120 × f16→f32 `[128,12]` RMSNorm γ, 120 × f32→**bf16** `[128,4096,12]`/`[128,256,12]` K/V (FA `prep_kv`), 2 × f32→f16 mask | **A** for SUB (F32/F32), STEP (UNARY, F32/F32) and the F32→F16 casts; **C** for the 120 f32→**BF16** K/V casts, which belong to the BF16 FlashAttention path the WebGPU backend does not support at all (§5 C1) — on `--no-fa` they are not built |
...
| GROUP_NORM | naf_gpu.cpp:62 `group_norm_affine` | 8 | [1024,1024,128,1] | 536.9 MB | `n_groups=8, eps=1e-5`; affine applied as separate MUL+ADD (ggml's group_norm has no built-in affine) | **C** — `GGML_OP_GROUP_NORM` has no case in the switch (confirmed absent from the fork too); the affine MUL/ADD that follow (naf_gpu.cpp:65-66) are independently **A**, but blocked on this. Small/well-understood kernel (group mean/var reduction) — §5/§7 item 2 (2026-09-20: lowered to `ggml_norm` over a `[W*H*C/8, 8]` reshape by `generic_lowering`; Metal uses the same re-expression by default since issue #55 because its native kernel is 32 threads/group) |
...
DiT's 3D interleaved-pair variant) is hand-built from elementwise ops rather than `ggml_rope_ext`
— worth knowing before assuming `GGML_OP_ROPE` WebGPU coverage matters here at all. Since PR #48 the
DiT's RoPE is `RESHAPE`/`PERMUTE`/`CONT`/`MUL`×4/`SUB`/`ADD`/`CONCAT` (all supported on WebGPU); the
`SET_ROWS`/`ARANGE` form described in the tables below is the pre-#48 graph.
...
| C2 | `ARANGE` (FA pad-mask ramp only since #48) | dit.cpp:104 | out `[Lk_pad]` up to ~17,664 | f32 | cont | trivial (≤70 KB), **2 occurrences/graph** (was 122 before #48) | Yes, trivially — output is a pure function of `Lk_pad`, no runtime data dependency | No | **Mostly resolved.** The 120 RoPE index builds are gone: #48 rewrote the rotation into a de-interleaved form that needs no index at all (§2's RoPE row). What is left is the pad-mask ramp, 2 per graph and only on the FA path; the same graph-construction fix applies (host-precompute + upload, as DINOv3's RoPE already does, dinov3.cpp:60-71). §7 item 1 |
...
| class | count | where |
|---|---|---|
| A | ~34 op-instances | DINOv3 linears/norms/unary/concat/reshape (S512 fully, S1024 mostly); DiT RMS_NORM/NORM/SUB/STEP/CPY/PAD and the #48 RoPE's CONT/CONCAT/MUL/ADD; SS decoder's norm/silu/add (not the conv itself); IM2COL/patch-embed |
...
| C | 6 distinct missing ops | `FLASH_ATTN_EXT`-with-BF16 (DiT default), `ARANGE` (DiT FA pad-mask; the RoPE's index build is gone since #48), `IM2COL_3D`/`CONV_3D` (SS decoder), `PAD_REFLECT_1D`, `GROUP_NORM`, `POOL_2D` (all three in NAF encoder) |
...
- **DiT (SS + both SLAT flows)**: C — `FLASH_ATTN_EXT` w/ BF16 K/V (default path, all three DiTs); `ARANGE` (FA pad-mask only, 2×/graph since #48; the RoPE no longer builds one).
...
- Must fix: DiT `FLASH_ATTN_EXT` BF16 gap (C1/B1 — F16 K/V with validated tolerance, or the exact
  chunked path retuned to the spec floor); DiT `ARANGE` (C2, cheap graph fix; only the 2 pad-mask ramps are left after #48); SS decoder's
...
1. **Graph-construction changes that need no new kernel** — highest value-per-effort, unblocks
   real testing immediately: host-precompute DiT's `ARANGE`-built pad-mask ramp (C2,
   mirrors DINOv3's already-working pattern; the RoPE index it also used to build is gone since #48); retune every existing chunk-budget constant
...
- **ARANGE** (C2): resolved for the DiT RoPE — at the time of this run by a host-built `rope_idx`
  input, and since #48 by dropping the index entirely (the de-interleaved rotation needs none);
