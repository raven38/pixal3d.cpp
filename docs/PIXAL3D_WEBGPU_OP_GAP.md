...
> **2026-09-20 更新**: DiT の 3D RoPE は de-interleaved レイアウトへ書き換えられ (`src/dit.cpp::apply_rope`, PR #48)、`ARANGE` / `SET_ROWS` / ゼロ埋め `SCALE` と `rope_idx` 入力は **DiT のグラフから消えた**。現在の RoPE は `RESHAPE`/`PERMUTE`/`CONT`/`MUL`×4/`SUB`/`ADD`/`CONCAT` のみで、いずれも WebGPU に実装がある (A)。以下の ARANGE / SET_ROWS 行のうち `apply_rope` に由来する分は歴史的記述で、`build_pad_mask` 由来の `ARANGE`/`STEP`/`CPY` だけが現存する。
